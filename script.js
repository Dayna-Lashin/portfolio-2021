...
var colors = ["#FF6B6B","#FFE66D","#4472CA"]; // array for the color of the shape

function createParticle(event){
    var x = event.clientX; // x coordinate of mouse click
    var y = event.clientY; // y coordinate of mouse click
    var color = Math.floor(Math.random() * 3); // random number 0-3 rounded down to nearest whole integer (3 because there are 3 colors in array)
    ...
    div.style.borderBottom = "10px solid " + colors[color]; // gives div a bottom border that's 10px, solid, and a color that corresponds with the random number in the color array
...
function getParticles(){
    var np = document.documentElement.clientWidth / 40; // divides viewable HTML element width by 40 (viewable includes padding only)
    particles.innerHTML = ""; // returns particle HTML content as empty string
    for (var i = 0; i < np; i++) { // FOR LOOP starting at 0 increasing by 1 for as long as it's less than variable np run the below
        ...
        var rndw = Math.floor(Math.random() * w ) + 1; // random number bewteen 0 - viewable width  - rounded down to nearest whole integer plus 1
        var rndh = Math.floor(Math.random() * h ) + 1; // random number bewteen 0 - viewable height  - rounded down to nearest whole integer +1
        var widthpt = Math.floor(Math.random() * 8) + 5; // random number between 0-8 rounded down to nearest whole integer +5
